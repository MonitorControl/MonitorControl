...
class Utils: NSObject {
    ...
    static func addSliderMenuItem(toMenu menu: NSMenu, forDisplay display: ExternalDisplay, command: DDC.Command, title: String, enabled: Bool = true) -> SliderHandler {
        let item = NSMenuItem()
    ...
    static func acquirePrivileges() {
        if ProcessInfo.processInfo.environment["skip_read_priviledges"] == nil && !readPrivileges(prompt: true) {
            let alert = NSAlert()
