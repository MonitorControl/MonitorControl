...
public class IntelDDC {
  ...
  static func send(request: inout IOI2CRequest, to framebuffer: io_service_t, errorRecoveryWaitTime: UInt32? = nil) -> Bool {
    ...
    if IntelDDC.framebufferDispatchGroups[framebuffer] == nil {
      IntelDDC.framebufferDispatchGroups[framebuffer] = (DispatchQueue(label: "ddc-framebuffer-\(framebuffer)"), DispatchGroup())
    }

    IntelDDC.sem.signal()

    let (queue, group) = IntelDDC.framebufferDispatchGroups[framebuffer]!
