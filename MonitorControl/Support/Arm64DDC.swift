...
class Arm64DDC: NSObject {
  ...
  private static func checkIfDiscouraged(ioregService: IOregService) -> Bool {
    ...
    // M1 Mac Mini & Macbook Pro HDMI is broken for DDC communication
    let ddcBlockedModels = [
      "Macmini9", // Mac mini M1 2020
      "MacBookPro18", // Macbook Pro M1 Pro/Max 2021 (14", 16")
    ]
    let isBlockedModel = ddcBlockedModels.contains { modelIdentifier.starts(with: $0) }
    if ioregService.transportDownstream == "HDMI", isBlockedModel {
      return true
    }
    return false
  }
